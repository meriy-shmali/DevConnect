...
const SidebarPanel = ({ title, icon, items, highlightedCommentId, showFilter, onClose, type, sort, setSort, postId, setCommentCount, currentUser }) => {
  ...
  // فتح الردود تلقائياً إذا كان الإشعار لرد فرعي
  useEffect(() => {
    if (!highlightedCommentId || !isComments || !items?.length) return;
    items.forEach(item => {
      const hasTargetReply = item.replies?.some(
        r => String(r.id) === String(highlightedCommentId)
      );
      if (hasTargetReply) {
        handleViewreply(item.id);
      }
    });
  }, [highlightedCommentId, isComments, items?.length]); // ✅ بدون logic في الـ dependency
          ...
          {items.length === 0 ? (
            ...
            items.map(item => (
              isComments ? (
                ...
                <HeaderPanel key={item.id} user={item} type={type} currentUserId={currentUser?.id} />
              )
            ))
          )}
        </div>
