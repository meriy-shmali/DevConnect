...
const FollowingModal = ({ isOpen, onClose, userId }) => {
  ...
  if (token) {
    try {
      ...
      console.error("Error decoding token in FollowingModal", e);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 transition-all duration-300 cursor-pointer" onClick={onClose}>
      ...
      >
        ...
        <div 
          onClick={(e) => e.stopPropagation()} 
          className="max-h-[50vh] md:max-h-[52vh] lg:max-h-[58vh] custom-scrollbar flex-1 overflow-y-auto  preview-scroll p-4 ml-3 pr-6 space-y-3"
        >
          {isLoading ? (
