...
import { motion, AnimatePresence } from 'framer-motion'; // ✅ إضافة الأنميشن
...
const SearchDropdown = () => {
  ...
  return (
    <div className="relative w-full max-w-full py-1" dir={isRtl ? 'rtl' : 'ltr'} ref={dropdownRef}>
      ...
      {/* ✅ استخدام AnimatePresence لمراقبة دخول وخروج القائمة مع الـ Exit Animation */}
      <AnimatePresence>
        {isOpen && (
         <motion.div 
initial={{ opacity: 0, y: -10, x: "-50%", scale: 0.98 }}
  ...
  /* 🛠️ الكلاسات المحدثة: أضفنا حماية الحواف والتجاوز max-w للابتوبات */
  className="absolute top-full mt-2 bg-white dark:bg-dark-post-background 
    left-1/2
    w-[95vw] sm:w-full min-w-[280px] sm:min-w-[450px] md:min-w-[550px] max-w-[calc(100vw-2rem)]
    shadow-2xl rounded-xl border border-gray-100 dark:border-gray-700 z-50 overflow-hidden"
>
                    ...
                    <>
                      ...
                      {recents.map((item, index) => (
                        <div 
                          ...
                          className="flex items-center justify-between py-2.5 px-4 md:px-6 hover:bg-gray-50 dark:hover:bg-gray-100/20 hover:rounded-lg cursor-pointer transition-colors"
                        >
                          ...
                          {activeTab === 'people' ? (
                            <>
                              ...
                              <div className="flex flex-col min-w-0">
                                ...
                                <div className="text-xs flex justify-start items-start text-gray-500 truncate">{item.specialization}</div>
                              </div>
                            </>
                          ) : (
                            <>
